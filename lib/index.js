            ...
            JSXOpeningElement: function JSXOpeningElement(path, state) {
                ...
                if (referencesImport(name, moduleSourceName, COMPONENT_NAMES)) {
                    var attributes = path.get('attributes').filter(function (attr) {
                        return attr.isJSXAttribute() || attr.isJSXSpreadAttribute();
                    });
                    var descriptor = createMessageDescriptor(descriptorProps, attributes.reduce(function (memo, attr) {
                        var arrayToAdd = void 0;
                        if (attr.isJSXSpreadAttribute()) {
                            var argument = attr.get('argument');
                            if (!argument.isObjectExpression()) {
                                return memo;
                            }
                            var properties = argument.get('properties');
                            ...
                            .filter(function (prop) {
                                return !prop.isSpreadProperty();
                            }).map(function (prop) {
                                return [prop.get('key'), prop.get('value')];
                            });
                        } else {
                            arrayToAdd = [[attr.get('name'), attr.get('value')]];
                        }
                    ...
                    if (descriptor.defaultMessage) {
                        ...
                        attributes.some(function (attr) {
                            if (attr.isJSXSpreadAttribute()) {
                                return attr.get('argument').get('properties').some(function (prop) {
